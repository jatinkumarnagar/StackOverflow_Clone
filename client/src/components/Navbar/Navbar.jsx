...
const Navbar = () => {
    ...
    const handleLogout = () => {
        dispatct({ type: 'LOGOUT' });
        navigate('/')
        dispatct(setCurrentUser(null))
    }

    const YourComponent = ({ User, handleLogout, dispatch }) => {
        useEffect(() => {
        ...
        }, [User?.token, handleLogout, dispatch]); // Include User?.token in the dependency array
